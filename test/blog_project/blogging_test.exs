defmodule BlogProject.BloggingTest do
  ...
  describe "blogs" do
    ...
    @invalid_attrs %{context: nil, name: nil}
    ...
    test "create_blog/1 with valid data creates a blog" do
      valid_attrs = %{context: "some context", name: "some name"}

      assert {:ok, %Blog{} = blog} = Blogging.create_blog(valid_attrs)
      assert blog.context == "some context"
    ...
    test "update_blog/2 with valid data updates the blog" do
      blog = blog_fixture()
      update_attrs = %{context: "some updated context", name: "some updated name"}

      assert {:ok, %Blog{} = blog} = Blogging.update_blog(blog, update_attrs)
      assert blog.context == "some updated context"
